...
- GPT API를 대화내용으로 학습시켜 끝말잇기 놀이를 이해하고 수행하도록 하고, 유저와 1:1로 끝말잇기 놀이를 진행할 수 있도록 합니다.
- 두 가지 언어(한국어, 영어)로 각각 학습시킨 데이터를 기반으로 끝말잇기 수행합니다.
...
- 사전 API를 구현한 뒤, GPT API와 사전 API에 리턴값이 필요하게 되었습니다. 왜냐하면 GPT API에서는 단어를 추출한 문자열 값이 필요했고, 해당 문자열을 사전 API에 인자로 전달해야 했기 때문입니다. 그 때문에 api함수를 fetch로 정의했는데, 응답을 받기 위해서는 함수를 `async`로 선언하여 리턴값을 받고, 버튼 이벤트 리스너에서 처리할 수 있도록 코딩했습니다. `async`으로 정의된 함수의 반환값이 `Promise`라는 것을 알게 되었습니다.

- 비동기 프로그래밍 이라고 하면 지레 겁을 먹게 되는 부분이 있었는데 JavaScript는 그런 부분들을 비교적 쉽게 접근할 수 있는 언어라고 느껴졌습니다.
